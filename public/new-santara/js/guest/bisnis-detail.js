...
function pembulatan(nilai, pembulat) {
  hasil = (Math.ceil(parseInt(nilai) % parseInt(pembulat)) == 0) ? Math.ceil(parseInt(nilai)) :
...
$(document).ready(function () {
  ...
  const invest = document.getElementById("jumlah_saham");
  ...
  $('#log-dividen').click(function () {
    $('#pembagianDividen').modal('show');
    $('#datadividen').scrollTop($('#datadividen')[0].scrollHeight);
  })
  ...
  if (btnbeli) {
    ...
    invest.addEventListener("keyup", function (e) {
      ...
      if (parseInt(this.value) <= 100 || this.value == '') {
        $('#jumlah_saham').val(100)
      }
      this.value = pembulatan(this.value, 100);
      ...
      if (!isNaN(total_price.value) && total_price.value > 0) {
        ...
        total_price.value = formatNumber(
          parseInt(total_price.value.replace(/\./g, ""))
        );
      } else {
        total_price.value = 0;
      }
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data.msg == 200) {
            if (
              data.type == "personal" &&
              parseInt(sisa_invest.innerHTML.replace(/\./g, "")) <
              parseInt(total_price.value.replace(/\./g, ""))
            ) {
              Swal.fire({
                ...
                html: `<table class="table table-borderless modal-emiten-detail">
                  ...
                  parseInt(max_invest.innerHTML)
                                ...
                                <tr>
                                    ...
                                    <td>:</td>
                                    <td>${formatRupiah(
                  parseInt(emitten_invest.innerHTML)
                )}</td>
                                </tr>             
                                <tr> 
                                    ...
                                    <td>:</td>
                                    <td>${formatRupiah(
                  parseInt(sisa_invest.innerHTML)
                )}</td>
                                </tr>             
                                <tr>
